...
### Added
- **SSTI scanner** (`ssti_scanner`, 8 scanners total): probes template-taking parameters across 10 engine families (Jinja2, Twig, ERB, Freemarker, Mako, Velocity, Smarty, Nunjucks) with arithmetic payloads and baseline-diffed detection — the evaluated marker must appear where the baseline did not have it (CWE-1336, critical)
- **SSTI exploit verifier** (`ssti_exploit`, 7 exploits total): confirms template injection with engine-specific payloads and fingerprints the engine family for the report; arithmetic-only payloads — never destructive expressions
- **CORS exploit verifier** (`cors_exploit`): deepens the `check_cors` header scan into a verified attack — replays the request with attacker origins (`https://evil-attacker.com`, `null`) and distinguishes credential-bearing reflection (high) from plain reflection/wildcard (medium)
- **Path-traversal exploit verifier** (`traversal_exploit`): read-only system-file probes (`/etc/passwd`, `/etc/hosts`, `win.ini`) with content-signature matching, covering encoded and dot-segment variants
- **spawn_agent multi-agent orchestration**: the Brain can spawn specialist sub-agents — `recon` (attack-surface mapping), `vuln` (scanner + probing + fuzzing), `exploit` (verification, still gated behind `--exploit` consent). Each sub-agent runs its own loop with its own iteration budget and a **restricted toolset** enforced in the shared dispatcher (one scope-enforcement point); recursion is blocked at depth 1; sub-agent token usage drains into the parent's run total; findings land in the shared scan context automatically. Methodology prompt gains a DELEGATE step
- **Five new agent tools** (15 tools total): `store_note` (persistent scratchpad that survives context trimming), `fuzz_param` (one-parameter fuzzing across SQLi/SSTI/XSS/traversal payload classes with baseline diffing — observations only, no auto-findings), `diff_responses` (status/length/sha256 comparison for blind-injection confirmation), `enum_paths` (~40 canonical sensitive paths, HEAD probes, concurrency-capped), `lookup_cwe` (OWASP Top 10 knowledge-base grounding for severity and remediation reasoning)
- **Eval scenarios for spawn_agent** (10 builtin scenarios total): `spawn_recon_delegation` (sub-agent runs a real loop and reports back) and `spawn_exploit_requires_consent` (exploit specialty refused without authorization) keep the orchestration regression-tested
...
### Fixed
- **URL-builder bug in all exploit verifiers**: `_build_test_url` derived the query separator from the *original* URL while re-encoding the full parameter set, producing malformed URLs like `/page&param=payload` for any finding whose URL already carried a query string — payload injection silently targeted the wrong resource. Affected `sqli_exploit`, `xss_exploit`, `ssrf_exploit`, `ssti_exploit`, `traversal_exploit`
